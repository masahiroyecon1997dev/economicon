...
class VariableEffectsEstimation(AbstractApi):
    ...
    SUPPORTED_STANDARD_ERROR_METHODS = [
        ...
        # 注意: 'cluster' はグループ情報が必要なため、この実装では除外
    ]
    ...
    def validate(self):
        try:
            ...
            # 標準誤差計算方法の検証
            if self.standard_error_method not in self.SUPPORTED_STANDARD_ERROR_METHODS:
                message = _(f"{self.param_names['standard_error_method']} must be one of: "
                          f"{', '.join(self.SUPPORTED_STANDARD_ERROR_METHODS)}")
    ...
    def execute(self) -> Dict:
        try:
            ...
            if self.standard_error_method == 'HAC':
                # HAC requires maxlags parameter
                cov_kwds = {'maxlags': 1}  # デフォルトでmaxlags=1を設定
            
            results = model.fit(
                cov_type=self.standard_error_method,
                cov_kwds=cov_kwds,
            ...
            for i, name in enumerate(param_names):
                ...
                params_info.append({
                    ...
                    'tValue': float(results.tvalues[i]) if self.use_t_distribution else float(results.tvalues[i]),
