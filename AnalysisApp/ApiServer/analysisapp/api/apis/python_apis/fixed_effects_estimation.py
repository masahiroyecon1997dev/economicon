...
from ..utilities.validator.common_validators import ValidationError, validate_required, validate_candidates
...
class FixedEffectsEstimation(AbstractApi):
    ...
    def execute(self) -> Dict:
        try:
            ...
            # データの準備
            all_columns = [self.dependent_variable] + self.explanatory_variables + [self.entity_id_column]
            ...
            if valid_entities.height == 0:
                message = _("No entities with multiple observations found. Fixed effects requires multiple time periods per entity.")
                raise ApiError(message)

            # 有効な個体のみを保持
            valid_entity_ids = valid_entities.select(self.entity_id_column).to_series().to_list()
            working_df = working_df.filter(pl.col(self.entity_id_column).is_in(valid_entity_ids))

            # 個体内平均を計算
            group_means = working_df.group_by(self.entity_id_column).agg([
                pl.mean(col).alias(f"{col}_mean") 
                for col in [self.dependent_variable] + self.explanatory_variables
            ])
            ...
            y_demeaned = working_df.select(
                (pl.col(self.dependent_variable) - pl.col(f"{self.dependent_variable}_mean")).alias("y_demeaned")
            ).to_series().to_numpy()

            x_demeaned_list = []
            for var in self.explanatory_variables:
                x_demeaned = working_df.select(
                    (pl.col(var) - pl.col(f"{var}_mean")).alias(f"{var}_demeaned")
                ).to_series().to_numpy()
            ...
            if self.standard_error_method == 'robust':
                ...
                # クラスター標準誤差（個体でクラスター）
                entity_ids = working_df.select(self.entity_id_column).to_series().to_numpy()
                model = model.get_robustcov_results(cov_type='cluster', groups=entity_ids)
            ...
            df_resid = n_obs - len(self.explanatory_variables) - n_entities + 1  # 固定効果を考慮

            model_stats = {
                'R2': float(model.rsquared),
                'adjustedR2': float(model.rsquared_adj),
                'fValue': float(model.fvalue) if hasattr(model, 'fvalue') else None,
                'fProbability': float(model.f_pvalue) if hasattr(model, 'f_pvalue') else None,
                'nObservations': n_obs,
...
def fixed_effects_estimation(
    ...
    Args:
        ...
        standard_error_method: 標準誤差の計算方法 ('normal', 'clustered', 'robust', 'hac')
