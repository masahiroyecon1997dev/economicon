...
from ..utilities.validator.common_validators import ValidationError, validate_candidates
...
class ConfidenceInterval(AbstractApi):
    ...
    def execute(self):
        try:
            ...
            if self.statistic_type == 'mean':
                statistic_value, ci_lower, ci_upper = self._calculate_mean_ci(data_array)
            elif self.statistic_type == 'median':
                statistic_value, ci_lower, ci_upper = self._calculate_median_ci(data_array)
            elif self.statistic_type == 'proportion':
                statistic_value, ci_lower, ci_upper = self._calculate_proportion_ci(data_array)
            elif self.statistic_type == 'variance':
                statistic_value, ci_lower, ci_upper = self._calculate_variance_ci(data_array)
            elif self.statistic_type == 'std':
                statistic_value, ci_lower, ci_upper = self._calculate_std_ci(data_array)
    ...
    def _calculate_mean_ci(self, data):
        ...
        return float(mean_val), float(mean_val - margin_of_error), float(mean_val + margin_of_error)
