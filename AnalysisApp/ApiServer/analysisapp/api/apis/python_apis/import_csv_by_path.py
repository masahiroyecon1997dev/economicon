...
from ..utilities.validator.input_validation_config import INPUT_VALIDATOR_CONFIG
...
class ImportCsvByPath(AbstractApi):
    ...
    def validate(self):
        ...
        try:
            ...
            if not self.table_name:
                raise ValidationError(_("Table name is required"))
            
            # ファイルパスの存在チェック
            if not os.path.exists(self.file_path):
                raise ValidationError(_("File does not exist: {path}").format(path=self.file_path))
            
            # ファイルが読み取り可能かチェック
            if not os.access(self.file_path, os.R_OK):
                raise ValidationError(_("File is not readable: {path}").format(path=self.file_path))
            
            # CSVファイルの拡張子チェック
            if not (self.file_path.lower().endswith('.csv') or self.file_path.lower().endswith('.tsv')):
                raise ValidationError(_("File must be a CSV or TSV file"))
            ...
            # 区切り文字のバリデーション（空文字や非常に長い文字列を防ぐ）
            if not self.separator:
                raise ValidationError(_("Separator cannot be empty"))
            
            if len(self.separator) > 5:
                raise ValidationError(_("Separator is too long (max 5 characters)"))
    ...
    def execute(self):
        ...
        try:
            ...
            df = pl.read_csv(self.file_path, separator=self.separator, encoding='utf8')
            ...
            message = _("Failed to parse CSV file: Invalid format or encoding.")
            raise ApiError(message) from e
...
def import_csv_by_path(file_path: str, table_name: str, separator: str = ',') -> Dict:
    """
