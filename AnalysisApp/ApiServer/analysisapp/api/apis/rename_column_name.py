...
from .utilities.validator.input_request_validators import (
    validate_rename_column_request,
)
from .data.tables_info import all_tables_info


class RenameColumnName(APIView):
    ...
    def post(self, request):
        try:
            ...
            # バリデーション
            validation_error = validate_rename_column_request(request_data)
            if validation_error:
            ...
            table_info = all_tables_info.pop(table_name)
            df = table_info.table
            new_df = df.rename({old_column_name: new_column_name})
