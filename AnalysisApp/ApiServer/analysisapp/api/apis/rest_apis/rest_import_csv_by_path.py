...
from ..python_apis.import_csv_by_path import import_csv_by_path
from ..python_apis.common_api_class import ApiError
from ..utilities.validator.common_validators import ValidationError


class ImportCsvByPath(APIView):
    """
    CSVファイルパス指定インポート用のREST APIクラス

    指定されたパスのCSVファイルを処理し、指定されたテーブル名でテーブルを作成します。
    """

    def post(self, request):
        """
        CSVファイルをパス指定でインポートしてテーブルを作成する
