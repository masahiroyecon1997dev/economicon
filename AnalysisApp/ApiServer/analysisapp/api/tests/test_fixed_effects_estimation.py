...
class TestApiFixedEffectsEstimation(APITestCase):
    ...
    def test_fixed_effects_estimation_success(self):
        ...
        x1_coeff = next(p['coefficient'] for p in parameters if p['variable'] == 'x1')
        x2_coeff = next(p['coefficient'] for p in parameters if p['variable'] == 'x2')
    ...
    def test_fixed_effects_estimation_robust_standard_errors(self):
        ...
        result = response_data['result']
        self.assertEqual(result['modelStatistics']['standardErrorMethod'], 'robust')
    ...
    def test_fixed_effects_estimation_clustered_standard_errors(self):
        ...
        result = response_data['result']
        self.assertEqual(result['modelStatistics']['standardErrorMethod'], 'clustered')
    ...
    def test_fixed_effects_estimation_hac_standard_errors(self):
        ...
        result = response_data['result']
        self.assertEqual(result['modelStatistics']['standardErrorMethod'], 'hac')
    ...
    def test_fixed_effects_estimation_entity_id_same_as_dependent(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response_data['code'], 'NG')
        self.assertIn("Entity ID column cannot be the same as dependent variable",
                      response_data['message'])
    ...
    def test_fixed_effects_estimation_entity_id_in_explanatory(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response_data['code'], 'NG')
        self.assertIn("Entity ID column cannot be included in explanatory variables",
                      response_data['message'])
