...
class TestApiListFiles(APITestCase):
    def setUp(self):
        # テスト用の一時ディレクトリを作成
    ...
    def test_list_files_missing_directory_path(self):
        """
    ...
    def test_list_files_file_sizes(self):
        """
        ...
        # test1.txtのサイズ確認
        test1_item = next(item for item in items if item['name'] == 'test1.txt')
        self.assertEqual(test1_item['size'], len('test content 1'))
        
        # test2.txtのサイズ確認
        test2_item = next(item for item in items if item['name'] == 'test2.txt')
        self.assertEqual(test2_item['size'], len('test content 2 with more data'))
        
        # サブディレクトリのサイズ確認
        subdir_item = next(item for item in items if item['name'] == 'subdir')
