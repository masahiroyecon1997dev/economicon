...
Write-Step "[5/9] API ファイルを resources/api/ へコピー"

$API_RESOURCES_DIR = Join-Path $TAURI_DIR "resources\api"

# 既存の api リソースをクリーン
if (Test-Path $API_RESOURCES_DIR) {
    Remove-Item -Recurse -Force $API_RESOURCES_DIR
}
New-Item -ItemType Directory -Path $API_RESOURCES_DIR | Out-Null

# main.py のコピー
Copy-Item (Join-Path $API_DIR "main.py") -Destination $API_RESOURCES_DIR -Force
Write-Success "main.py → resources\api\main.py"

# economicon/ パッケージのコピー（__pycache__・tests を除外）
$economiconSrc = Join-Path $API_DIR "economicon"
$economiconDst = Join-Path $API_RESOURCES_DIR "economicon"
...
Write-Success "economicon/ → resources\api\economicon\"
