...
頻出タスクのステップバイステップワークフローを [skills/](skills/) で定義:

| スキル             | 用途                                                                            |
| ------------------ | ------------------------------------------------------------------------------- |
| `new-api-endpoint` | FastAPI エンドポイント新規追加（スキーマ→サービス→ルーター→i18n→型同期→テスト） |
| `new-screen`       | React 画面新規追加（型確認→ストア→コンポーネント→フォーム→i18n→テスト）         |
