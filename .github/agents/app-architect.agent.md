...
- **API型・バリデーション**: `app/src/api/model/`（型）と `app/src/api/zod/`（Zodスキーマ）はOrval + OpenAPIから自動生成。手動再定義・再実装禁止。フォームバリデーションでもこれらをインポートして使う
...
- APIリクエスト/レスポンスのZodスキーマは `src/api/zod/` の自動生成物を使う。`src/schemas/` には画面固有のフォームスキーマのみ配置し、APIスキーマを手動で再定義しない
