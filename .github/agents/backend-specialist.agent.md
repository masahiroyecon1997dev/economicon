...
### 1. クラスベースのサービス設計 (DataOperation)

全てのビジネスロジックは `DataOperation` プロトコルに適合したクラスとして実装します。
Protocolによる構造的部分型（structural subtyping）を使用するため、継承は不要です。
...
- ユーザーに表示されるメッセージは必ず `economicon.i18n.translation.gettext as _` を使用し、翻訳可能な形式で記述します。
...
- `DataOperation` プロトコルに適合しない生の関数のみでのビジネスロジック実装。
