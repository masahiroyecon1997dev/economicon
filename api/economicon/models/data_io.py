...
class ImportCsvByPathRequestBody(BaseModel):
    ...
    separator: str = Field(
        default=",", description="区切り文字", min_length=1, max_length=10
    )

    model_config = ConfigDict(populate_by_name=True)


class ImportExcelByPathRequestBody(BaseModel):
    ...
    sheet_name: Optional[str] = Field(
        default=None, alias="sheetName", description="シート名", max_length=255
    )

    model_config = ConfigDict(populate_by_name=True)


class ImportParquetByPathRequestBody(BaseModel):
...
class ExportCsvByPathRequestBody(BaseModel):
