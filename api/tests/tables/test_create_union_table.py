...
def tables_store():
    ...
    different_table_df = pl.DataFrame(
        {"id": [9, 10], "username": ["user1", "user2"], "score": [100, 200]}
    )
...
def test_union_table_name_empty(client, tables_store):
    ...
    assert (
        "unionTableName は1文字以上である必要があります。"
...
def test_missing_request_fields(client, tables_store):
    ...
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_CONTENT
    # # tableNamesが欠けている場合
    payload = {"unionTableName": "UnionTable", "columnNames": ["id", "name"]}
    response = client.post(
        "/api/table/create-union",
        json=payload,
    )
...
def test_create_union_table_pydantic_empty_union_table_name(
    client, tables_store
):
    """
...
def test_create_union_table_pydantic_missing_union_table_name(
    client, tables_store
):
    """
