...
def test_logit_success(client, tables_store):
    ...
    resp = client.post(URL_REGRESSION, json=logit_payload())
...
def test_logit_response_structure(client, tables_store):
    ...
    output = _get_output(client, logit_payload())
...
def test_logit_coefficients_numerical(client, tables_store):
    ...
    params = _get_output(client, logit_payload())["parameters"]
...
def test_logit_pseudo_r2_range(client, tables_store):
    ...
    model_stats = _get_output(client, logit_payload())["modelStatistics"]
...
def test_logit_robust_se(client, tables_store):
    ...
    resp = client.post(URL_REGRESSION, json=logit_payload(se_method="robust"))
    assert resp.status_code == status.HTTP_200_OK
