...
from ..utils.validator.common_validators import ValidationError, validate_integer
...
class CreateSimulationDataTable(AbstractApi):
    ...
    def validate(self):
        try:
            ...
            validate_new_table_name(
                self.table_name, table_name_list, self.param_names["table_name"]
            )

            # 行数の検証
            validate_integer(
                self.table_number_of_rows, self.param_names["table_number_of_rows"]
            )
            ...
            if len(self.column_settings) == 0:
                raise ValidationError(_("At least one column setting is required."))
    ...
    def _validate_column_setting(
        ...
        if data_type not in ["distribution", "fixed"]:
            raise ValidationError(_("Data type must be 'distribution' or 'fixed'."))
        ...
        if data_type == "distribution":
            ...
            if "distributionParams" not in column_setting:
                raise ValidationError(
                    _("Distribution parameters are required for distribution data.")
                )

            distribution_params = column_setting["distributionParams"]
            validate_distribution_params(distribution_type, distribution_params)

        # 固定値データの場合の検証
        elif data_type == "fixed":
            if "fixedValue" not in column_setting:
                raise ValidationError(_("Fixed value is required for fixed data."))
...
def create_simulation_data_table(
    table_name: str, table_number_of_rows: int, column_settings: List[Dict[str, Any]]
) -> Dict:
