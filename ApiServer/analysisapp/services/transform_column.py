...
class TransformColumn(AbstractApi):
    ...
    def validate(self):
        try:
            ...
            validate_new_column_name(
                self.new_column_name, column_name_list, self.param_names["new_column"]
            )
            ...
            if self.transform_method == "root" and self.root_index is not None:
                if not (
                    isinstance(self.root_index, (int, float)) or self.root_index == 0
                ):
                    raise ValidationError(
    ...
    def execute(self):
        try:
            ...
            if self.transform_method == "log":
                ...
                root_index = self.root_index if (self.root_index is not None) else 2.0
            ...
            # Return result
            result = {"tableName": self.table_name, "columnName": self.new_column_name}
            return result
