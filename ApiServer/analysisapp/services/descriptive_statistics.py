...
class DescriptiveStatistics(AbstractApi):
    ...
    def __init__(
        self, table_name: str, column_name_list: List[str], statistics: List[str]
    ):
    ...
    def validate(self):
        try:
            ...
            for column_name in self.column_name_list:
                validate_existed_column_name(
                    column_name, column_name_list, self.param_names["column_names"]
                )

            # 統計の種類をバリデーション
            if not self.statistics:
                raise ValidationError(_("statistics is required"))
            for stat in self.statistics:
                if stat not in self.AVAILABLE_STATISTICS:
                    available = list(self.AVAILABLE_STATISTICS.keys())
                    raise ValidationError(
                        _(
                            "statistics '{}'はサポートされていません。利用可能: {}"
    ...
    def execute(self):
        try:
            ...
            # 結果を返す
            result = {"tableName": self.table_name, "statistics": result}
            return result
