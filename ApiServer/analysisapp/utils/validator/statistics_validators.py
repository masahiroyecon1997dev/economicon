    ...
    match distribution_type:
        case 'uniform':
            if 'low' not in params or 'high' not in params:
                raise ValidationError(_("Uniform distribution requires "
                                      "'low' and 'high' parameters"))
            validate_number(params['low'], 'low')
            validate_number(params['high'], 'high')
            if params['low'] >= params['high']:
                raise ValidationError(_("For uniform distribution, "
                                      "'low' must be less than 'high'"))
        case 'exponential':
            if 'scale' not in params:
                raise ValidationError(_("Exponential distribution requires "
                                      "'scale' parameter"))
            validate_number(params['scale'], 'scale')
            if params['scale'] <= 0:
                raise ValidationError(_("For exponential distribution, "
                                      "'scale' must be positive"))
        case 'normal':
            if 'loc' not in params or 'scale' not in params:
                raise ValidationError(_("Normal distribution requires "
                                      "'loc' and 'scale' parameters"))
            validate_number(params['loc'], 'loc')
            validate_number(params['scale'], 'scale')
            if params['scale'] <= 0:
                raise ValidationError(_("For normal distribution, "
                                      "'scale' must be positive"))
        case 'gamma':
            if 'shape' not in params or 'scale' not in params:
                raise ValidationError(_("Gamma distribution requires "
                                      "'shape' and 'scale' parameters"))
            validate_number(params['shape'], 'shape')
            validate_number(params['scale'], 'scale')
            if params['shape'] <= 0 or params['scale'] <= 0:
                raise ValidationError(_("For gamma distribution, "
                                      "'shape' and 'scale' must be "
                                      "positive"))
        case 'beta':
            if 'a' not in params or 'b' not in params:
                raise ValidationError(_("Beta distribution requires "
                                      "'a' and 'b' parameters"))
            validate_number(params['a'], 'a')
            validate_number(params['b'], 'b')
            if params['a'] <= 0 or params['b'] <= 0:
                raise ValidationError(_("For beta distribution, "
                                      "'a' and 'b' must be positive"))
        case 'weibull':
            if 'a' not in params or 'scale' not in params:
                raise ValidationError(_("Weibull distribution requires 'a' "
                                      "and 'scale' parameters"))
            validate_number(params['a'], 'a')
            if params['a'] <= 0:
                raise ValidationError(_("For weibull distribution, "
                                      "'a' must be positive"))
            validate_number(params['scale'], 'scale')
            if params['scale'] > 0:
                raise ValidationError(_("For weibull distribution, "
        ...
        case 'lognormal':
            if 'mean' not in params or 'sigma' not in params:
                raise ValidationError(_("Lognormal distribution requires "
                                      "'mean' and 'sigma' parameters"))
            validate_number(params['mean'], 'mean')
            validate_number(params['sigma'], 'sigma')
            if params['sigma'] <= 0:
                raise ValidationError(_("For lognormal distribution, "
                                      "'sigma' must be positive"))
        case 'binomial':
            if 'n' not in params or 'p' not in params:
                raise ValidationError(_("Binomial distribution requires "
                                      "'n' and 'p' parameters"))
            validate_integer(params['n'], 'n')
            validate_number(params['p'], 'p')
            if params['n'] <= 0:
                raise ValidationError(_("For binomial distribution, "
                                      "'n' must be positive"))
            if not (0 <= params['p'] <= 1):
                raise ValidationError(_("For binomial distribution, "
        ...
        case 'bernoulli':
            if 'p' not in params:
                raise ValidationError(_("Bernoulli distribution requires "
                                      "'p' parameter"))
            validate_number(params['p'], 'p')
            if not (0 <= params['p'] <= 1):
                raise ValidationError(_("For bernoulli distribution, "
                                      "'p' must be between 0 and 1"))
        case 'poisson':
            if 'lam' not in params:
                raise ValidationError(_("Poisson distribution requires "
                                      "'lam' parameter"))
            validate_number(params['lam'], 'lam')
            if params['lam'] <= 0:
                raise ValidationError(_("For poisson distribution, "
                                      "'lam' must be positive"))
        case 'geometric':
            if 'p' not in params:
                raise ValidationError(_("Geometric distribution requires "
                                      "'p' parameter"))
            validate_number(params['p'], 'p')
            if not (0 < params['p'] <= 1):
                raise ValidationError(_("For geometric distribution, "
                                      "'p' must be between 0 and 1 "
                                      "(exclusive of 0)"))
        case 'hypergeometric':
            if 'N' not in params or 'K' not in params or 'n' not in params:
                raise ValidationError(_("Hypergeometric distribution requires "
                                      "'N', 'K', and 'n' parameters"))
            validate_integer(params['N'], 'N')
            validate_integer(params['K'], 'K')
            validate_integer(params['n'], 'n')
            if params['N'] <= 0 or params['K'] <= 0 or params['n'] <= 0:
                raise ValidationError(_("For hypergeometric "
                                      "distribution, 'N', 'K', "
