...
test.describe("01: CSV 取り込み → 列メニュー → データメニュー → 保存", async () => {
  ...
  // =========================================================================
  test("Step 2: 列メニュー — 昇順ソート・降順ソート", async ({
    playwright,
  }) => {
    const page = await setupTauriApp(playwright);
    // 前提: Step 1 でインポート済みのデータが存在すること
    // このテストは独立して実行するため、サイドバーからテーブルを開く
    const tableTab = page.getByRole("button", { name: TABLE_NAME });
    ...
    await expect(
      page.getByRole("columnheader", { name: TARGET_COL }),
    ).toBeVisible();
  ...
  test("Step 3: 列メニュー — 列名変更（value → value_renamed）", async ({
    ...
    // ダイアログで新しい列名を入力して変更
    await fillDialogAndSubmit(page, "value_renamed", /^変更$|^Rename$/i);
  ...
  // STEP 5: 列メニュー — 型変換
  // =========================================================================
  test("Step 5: 列メニュー — 型変換（Float64 → Int64）", async ({
    playwright,
  }) => {
    const page = await setupTauriApp(playwright);
    ...
    const headerName = (await page
      .getByRole("columnheader", { name: /\bvalue_renamed\b/ })
      .isVisible())
      ? "value_renamed"
      : TARGET_COL;
    ...
    const int64Option = page.getByRole("option", { name: /Int|整数/i });
    await int64Option.waitFor({ state: "visible" });
    await int64Option.click();

    // 新列名（デフォルトのまま Submit）
    await dialog.getByRole("button", { name: /^変換$|^Cast$/i }).click();
    await expect(dialog).toBeHidden();

    // テーブルにまだ列が表示されていること
    await expect(
      page.getByRole("columnheader", { name: `${headerName}_cast` }),
    ).toBeVisible();
  });
  ...
  // =========================================================================
  test("Step 6: 列メニュー — 変換列の追加（対数変換）", async ({
    playwright,
  }) => {
    const page = await setupTauriApp(playwright);
    ...
    await openColumnMenu(page, headerName);
    await clickColumnMenuItem(page, /変換列を追加|Add Transform Column/i);
  ...
  // =========================================================================
  test("Step 7: 列メニュー — ダミー変数の追加", async ({ playwright }) => {
    const page = await setupTauriApp(playwright);
    ...
    await openColumnMenu(page, headerName);
    await clickColumnMenuItem(page, /ダミー変数を追加|Add Dummy Column/i);
  ...
  // =========================================================================
  test("Step 8: 列メニュー — ラグ列の追加（periods=-1）", async ({
    playwright,
  }) => {
    const page = await setupTauriApp(playwright);
    ...
    await clickColumnMenuItem(
      page,
      /ラグ・リード列を追加|Add Lag\/Lead Column/i,
    );
    ...
    // Periods に -1 を入力
    const periodsInput = dialog.locator("#lag-periods");
  ...
  test("Step 9: 列メニュー — シミュレーション列を追加（正規分布）", async ({
    ...
    // 分布を正規分布に変更（RadioTagGroup: radio ボタンで選択）
    const normalRadio = dialog.getByRole("radio", {
      name: /^(正規分布|Normal)$/i,
    });
  ...
  test("Step 10: 列メニュー — 列を削除（sim_normal を削除）", async ({
    ...
    if (await dialog.isVisible()) {
      await dialog.getByRole("button", { name: /削除する|Delete/i }).click();
      await expect(dialog).toBeHidden();
    }

    // 削除した列がヘッダーから消えていること
    await expect(
      page.getByRole("columnheader", { name: colToDelete }),
    ).toBeHidden();
  });
  ...
  // =========================================================================
  test("Step 11: データメニュー — データ名変更", async ({ playwright }) => {
    const page = await setupTauriApp(playwright);
    ...
    await dialog.getByRole("button", { name: /^変更$|^Rename$/i }).click();
    await expect(dialog).toBeHidden();
  ...
  // =========================================================================
  test("Step 12: データメニュー — データを複製", async ({ playwright }) => {
    const page = await setupTauriApp(playwright);
