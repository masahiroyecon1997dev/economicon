 ...
 */
export const CreateGroupStatisticsTableHeader = zod.object({
  "X-Auth-Token": zod.string().optional().describe('Tauri 起動時に生成された認証トークン')
})
...
export const CreateGroupStatisticsTableBody = zod.object({
  "tableName": zod.string().min(1).describe('集計対象の元テーブル名。'),
  "groupByColumns": zod.array(zod.string().min(1).describe('カラム名')).min(1).describe('グループ化キーとする列名のリスト。浮動小数点型（Float32\/Float64）の列は指定不可。'),
  "statColumns": zod.array(zod.string().min(1).describe('カラム名')).min(1).describe('統計量を計算する対象列名のリスト。groupByColumns との重複は不可。'),
  "statistics": zod.array(zod.enum(['mean', 'median', 'mode', 'variance', 'std_dev', 'range', 'iqr', 'count', 'null_count', 'null_ratio', 'population_variance', 'min', 'max', 'skewness', 'kurtosis'])).min(1).describe('計算する統計量のリスト。（mean: 平均、median: 中央値、mode: 最頻値、variance: 不偏分散、std_dev: 標準偏差、range: 範囲、iqr: 四分位数範囲、count: 有効サンプル数、null_count: null数、null_ratio: null割合、population_variance: 母分散、min: 最小値、max: 最大値、skewness: 歪度、kurtosis: 超過尖度）'),
  "newTableName": zod.string().min(1).describe('結果を格納する新規テーブル名。')
}).describe('GroupBy 統計テーブル作成リクエスト')

export const createGroupStatisticsTableResponseCodeDefault = `OK`;

export const CreateGroupStatisticsTableResponse = zod.object({
  "code": zod.string().default(createGroupStatisticsTableResponseCodeDefault).describe('レスポンスコード'),
  "result": zod.object({
  "tableName": zod.string().describe('新規作成された GroupBy 統計テーブルの名前。')
}).describe('処理結果')
})
