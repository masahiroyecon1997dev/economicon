...
// Fixture
// ---------------------------------------------------------------------------
const RE_REGRESSION_RESULT = { resultId: "re-r-001" };

const RE_ANALYSIS_DETAIL = {
  id: "re-r-001",
  name: "RE 1",
  description: "",
  ...
  resultData: {
    ...
    regressionResult: "Random Effects (GLS)",
    parameters: [
      {
        variable: "const",
        coefficient: 1.72,
        ...
        pValue: 0.379,
        tValue: 0.88,
        confidenceIntervalLower: -2.15,
        confidenceIntervalUpper: 5.59,
      },
      {
        variable: "x1",
        coefficient: 3.0,
...
// Tests
// ---------------------------------------------------------------------------
describe("RERegressionForm", () => {
  it("タイトルと説明を表示する", () => {
    render(<RERegressionForm onCancel={onCancel} />);

    expect(screen.getByText("RERegressionForm.Title")).toBeInTheDocument();
    expect(
      screen.getByText("RERegressionForm.Description"),
    ).toBeInTheDocument();
  });
  ...
  // =========================================================================
  describe("API成功時", () => {
    it("method='re' で regression を呼び出し結果タブを開く", async () => {
      mockApi.regression.mockResolvedValue({
        code: "OK",
        result: RE_REGRESSION_RESULT,
      });
      mockApi.getAnalysisResult.mockResolvedValue({
        code: "OK",
        result: RE_ANALYSIS_DETAIL,
      });
