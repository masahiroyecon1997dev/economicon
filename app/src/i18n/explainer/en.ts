...
export const enExplainer = {
  ...
  group_statistics: {
    ...
    description:
      "Group-level statistics break the data down by a categorical grouping variable and compute summary statistics separately for each group. This lets you quickly compare subgroups — e.g., men vs. women, regions, treatment vs. control.\n\nChecking group balance is a critical pre-analysis step. In causal inference settings (e.g., program evaluation), verifying that treatment and control groups have similar baseline characteristics is essential for the validity of the analysis.\n\nGroup statistics also serve as an exploratory tool for detecting subgroup-specific patterns and potential Heterogeneous Treatment Effects (HTE) that would be masked by overall averages.",
    sections: [
      {
        heading: "Checking for Missing-Data Patterns and Selection Bias",
        body: "Comparing null_count and null_ratio across groups reveals whether missingness is systematically concentrated in particular subgroups. If a specific group has a disproportionately high missing rate, that group may be systematically excluded from estimation, introducing selection bias.\n\nWhen missingness is not Missing Completely At Random (MCAR) — particularly if it is Missing Not At Random (MNAR), meaning the probability of being missing depends on the unobserved outcome itself — simple listwise deletion produces biased estimates.",
      },
      {
        heading: "Group Differences ≠ Causal Effects",
        body: "Even a statistically significant group difference does not imply that group membership causes the difference. As long as confounders simultaneously influence both the grouping variable and the outcome, any observed group difference is merely an observational correlation.\n\nTo make causal claims from group differences, appropriate causal inference methods are needed: Difference-in-Differences (DiD), Regression Discontinuity Design (RDD), propensity score matching (PSM), or randomized experiments.",
      },
    ],
  },
  // ---------------------------------------------------------------------------
