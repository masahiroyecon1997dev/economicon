/**
 * C-10 グループ別統計量 — Remotion コンポジション
 *
 * 素材: captured/c10/frames/0001.jpg … NNNN.jpg + meta.json
 *
 * フレーム構成:
...
export const calculateC10Metadata: CalculateMetadataFunction<
  C10Props
> = async ({ props }) => {
  try {
    const res = await fetch(staticFile("c10/meta.json"));
...
export const C10GroupStatistics: React.FC<C10Props> = ({
  lang = "ja",
  _meta,
}) => {
  const t = I18N[lang].C10;
  ...
  if (!_meta) {
    return (
      ...
      >
        ...
        >
          {"pnpm capture:c10 を実行してください"}
          <br />
          {"(cd video/playwright && pnpm capture:c10)"}
        </p>
      </AbsoluteFill>
    );
  }
  ...
  return (
    <AbsoluteFill>
      ...
      <Sequence from={recordingFrom} durationInFrames={recordingFrames}>
        <FrameSequence sceneId="c10" meta={_meta} />
      </Sequence>
