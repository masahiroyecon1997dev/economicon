/**
 * C-05 列フィルタ・型変換 — Remotion コンポジション
 *
 * 素材: captured/c05/frames/0001.jpg … NNNN.jpg + meta.json
...
export const calculateC05Metadata: CalculateMetadataFunction<
  C05Props
> = async ({ props }) => {
  try {
    const res = await fetch(staticFile("c05/meta.json"));
...
export const C05ColumnFilterCast: React.FC<C05Props> = ({
  lang = "ja",
  _meta,
}) => {
  const t = I18N[lang].C05;
  ...
  if (!_meta) {
    return (
      ...
      >
        ...
        >
          {"pnpm capture:c05 を実行してください"}
          <br />
          {"(cd video/playwright && pnpm capture:c05)"}
        </p>
      </AbsoluteFill>
    );
  }
  ...
  return (
    <AbsoluteFill>
      ...
      <Sequence from={recordingFrom} durationInFrames={recordingFrames}>
        <FrameSequence sceneId="c05" meta={_meta} />
      </Sequence>
