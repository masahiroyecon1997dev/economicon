/**
 * C-04 テーブルの Union（縦結合） — Remotion コンポジション
 *
 * 素材: captured/c04/frames/0001.jpg … NNNN.jpg + meta.json
 *
 * フレーム構成:
...
const FALLBACK_RECORDING_FRAMES = FPS * 45; // 45s
...
export const calculateC04Metadata: CalculateMetadataFunction<
  C04Props
> = async ({ props }) => {
  try {
    const res = await fetch(staticFile("c04/meta.json"));
...
export const C04Union: React.FC<C04Props> = ({ lang = "ja", _meta }) => {
  const t = I18N[lang].C04;
  ...
  if (!_meta) {
    return (
      ...
      >
        ...
        >
          {"pnpm capture:c04 を実行してください"}
          <br />
          {"(cd video/playwright && pnpm capture:c04)"}
        </p>
      </AbsoluteFill>
    );
  }
  ...
  return (
    <AbsoluteFill>
      ...
      <Sequence from={recordingFrom} durationInFrames={recordingFrames}>
        <FrameSequence sceneId="c04" meta={_meta} />
      </Sequence>
