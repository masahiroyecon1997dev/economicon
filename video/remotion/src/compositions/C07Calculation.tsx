/**
 * C-07 計算列の追加 — Remotion コンポジション
 *
 * 素材: captured/c07/frames/0001.jpg … NNNN.jpg + meta.json
...
export const calculateC07Metadata: CalculateMetadataFunction<
  C07Props
> = async ({ props }) => {
  try {
    const res = await fetch(staticFile("c07/meta.json"));
...
export const C07Calculation: React.FC<C07Props> = ({ lang = "ja", _meta }) => {
  const t = I18N[lang].C07;
  ...
  if (!_meta) {
    return (
      ...
      >
        ...
        >
          {"pnpm capture:c07 を実行してください"}
          <br />
          {"(cd video/playwright && pnpm capture:c07)"}
        </p>
      </AbsoluteFill>
    );
  }
  ...
  return (
    <AbsoluteFill>
      ...
      <Sequence from={recordingFrom} durationInFrames={recordingFrames}>
        <FrameSequence sceneId="c07" meta={_meta} />
      </Sequence>
