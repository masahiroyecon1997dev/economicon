/**
 * C-02 Excel / Parquet のインポート — Remotion コンポジション
 *
 * 素材: captured/c02/frames/0001.jpg … NNNN.jpg + meta.json
 *
 * フレーム構成:
...
const ENDING_FRAMES = 90; // 3s
const FALLBACK_RECORDING_FRAMES = FPS * 60; // 60s (2 formats)
...
export const calculateC02Metadata: CalculateMetadataFunction<
  C02Props
> = async ({ props }) => {
  try {
    const res = await fetch(staticFile("c02/meta.json"));
...
export const C02ExcelParquetImport: React.FC<C02Props> = ({
  lang = "ja",
  _meta,
}) => {
  const t = I18N[lang].C02;
  ...
  if (!_meta) {
    return (
      ...
      >
        ...
        >
          {"pnpm capture:c02 を実行してください"}
          <br />
          {"(cd video/playwright && pnpm capture:c02)"}
        </p>
      </AbsoluteFill>
    );
  }
  ...
  return (
    <AbsoluteFill>
      ...
      <Sequence from={recordingFrom} durationInFrames={recordingFrames}>
        <FrameSequence sceneId="c02" meta={_meta} />
      </Sequence>
