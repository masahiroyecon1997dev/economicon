/**
 * C-11 相関行列 — Remotion コンポジション
 *
 * 素材: captured/c11/frames/0001.jpg … NNNN.jpg + meta.json
 *
 * フレーム構成:
...
export const calculateC11Metadata: CalculateMetadataFunction<
  C11Props
> = async ({ props }) => {
  try {
    const res = await fetch(staticFile("c11/meta.json"));
...
export const C11CorrelationMatrix: React.FC<C11Props> = ({
  lang = "ja",
  _meta,
}) => {
  const t = I18N[lang].C11;
  ...
  if (!_meta) {
    return (
      ...
      >
        ...
        >
          {"pnpm capture:c11 を実行してください"}
          <br />
          {"(cd video/playwright && pnpm capture:c11)"}
        </p>
      </AbsoluteFill>
    );
  }
  ...
  return (
    <AbsoluteFill>
      ...
      <Sequence from={recordingFrom} durationInFrames={recordingFrames}>
        <FrameSequence sceneId="c11" meta={_meta} />
      </Sequence>
