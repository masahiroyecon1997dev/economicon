/**
 * C-03 テーブルの Join（結合） — Remotion コンポジション
 *
 * 素材: captured/c03/frames/0001.jpg … NNNN.jpg + meta.json
 *
 * フレーム構成:
...
export const calculateC03Metadata: CalculateMetadataFunction<
  C03Props
> = async ({ props }) => {
  try {
    const res = await fetch(staticFile("c03/meta.json"));
...
export const C03Join: React.FC<C03Props> = ({ lang = "ja", _meta }) => {
  const t = I18N[lang].C03;
  ...
  if (!_meta) {
    return (
      ...
      >
        ...
        >
          {"pnpm capture:c03 を実行してください"}
          <br />
          {"(cd video/playwright && pnpm capture:c03)"}
        </p>
      </AbsoluteFill>
    );
  }
  ...
  return (
    <AbsoluteFill>
      ...
      <Sequence from={recordingFrom} durationInFrames={recordingFrames}>
        <FrameSequence sceneId="c03" meta={_meta} />
      </Sequence>
