/**
 * C-22 信頼区間シミュレーション — Remotion コンポジション
 *
 * 素材: captured/c22/frames/0001.jpg … NNNN.jpg + meta.json
 *
 * フレーム構成:
...
export const calculateC22Metadata: CalculateMetadataFunction<
  C22Props
> = async ({ props }) => {
  try {
    const res = await fetch(staticFile("c22/meta.json"));
...
export const C22CiSimulation: React.FC<C22Props> = ({ lang = "ja", _meta }) => {
  const t = I18N[lang].C22;
  ...
  return (
    <AbsoluteFill>
      ...
      <Sequence from={TITLE_FRAMES} durationInFrames={recordingFrames}>
        {_meta ? (
          <FrameSequence sceneId="c22" meta={_meta} />
        ) : (
          <AbsoluteFill
            style={{
              background: "#000",
