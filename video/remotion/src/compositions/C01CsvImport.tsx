/**
 * C-01 CSV ファイルのインポート — Remotion コンポジション
 *
 * 素材: captured/c01/frames/0001.jpg … NNNN.jpg + meta.json
 *
 * フレーム構成:
...
export const calculateC01Metadata: CalculateMetadataFunction<
  C01Props
> = async ({ props }) => {
  try {
    const res = await fetch(staticFile("c01/meta.json"));
...
export const C01CsvImport: React.FC<C01Props> = ({ lang = "ja", _meta }) => {
  const t = I18N[lang].C01;
  ...
  if (!_meta) {
    return (
      ...
      >
        ...
        >
          {"pnpm capture:c01 を実行してください"}
          <br />
          {"(cd video/playwright && pnpm capture:c01)"}
        </p>
      </AbsoluteFill>
    );
  }
  ...
  return (
    <AbsoluteFill>
      ...
      <Sequence from={recordingFrom} durationInFrames={recordingFrames}>
        <FrameSequence sceneId="c01" meta={_meta} />
      </Sequence>
