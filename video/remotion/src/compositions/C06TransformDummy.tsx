/**
 * C-06 変換列・ダミー変数の追加 — Remotion コンポジション
 *
 * 素材: captured/c06/frames/0001.jpg … NNNN.jpg + meta.json
...
export const calculateC06Metadata: CalculateMetadataFunction<
  C06Props
> = async ({ props }) => {
  try {
    const res = await fetch(staticFile("c06/meta.json"));
...
export const C06TransformDummy: React.FC<C06Props> = ({
  lang = "ja",
  _meta,
}) => {
  const t = I18N[lang].C06;
  ...
  if (!_meta) {
    return (
      ...
      >
        ...
        >
          {"pnpm capture:c06 を実行してください"}
          <br />
          {"(cd video/playwright && pnpm capture:c06)"}
        </p>
      </AbsoluteFill>
    );
  }
  ...
  return (
    <AbsoluteFill>
      ...
      <Sequence from={recordingFrom} durationInFrames={recordingFrames}>
        <FrameSequence sceneId="c06" meta={_meta} />
      </Sequence>
