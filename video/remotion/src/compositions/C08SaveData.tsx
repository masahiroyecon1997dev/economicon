/**
 * C-08 データの保存 — Remotion コンポジション
 *
 * 素材: captured/c08/frames/0001.jpg … NNNN.jpg + meta.json
...
export const calculateC08Metadata: CalculateMetadataFunction<
  C08Props
> = async ({ props }) => {
  try {
    const res = await fetch(staticFile("c08/meta.json"));
...
export const C08SaveData: React.FC<C08Props> = ({ lang = "ja", _meta }) => {
  const t = I18N[lang].C08;
  ...
  if (!_meta) {
    return (
      ...
      >
        ...
        >
          {"pnpm capture:c08 を実行してください"}
          <br />
          {"(cd video/playwright && pnpm capture:c08)"}
        </p>
      </AbsoluteFill>
    );
  }
  ...
  return (
    <AbsoluteFill>
      ...
      <Sequence from={recordingFrom} durationInFrames={recordingFrames}>
        <FrameSequence sceneId="c08" meta={_meta} />
      </Sequence>
