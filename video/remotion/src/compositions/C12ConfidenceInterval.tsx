/**
 * C-12 信頼区間の計算 — Remotion コンポジション
 *
 * 素材: captured/c12/frames/0001.jpg … NNNN.jpg + meta.json
 *
 * フレーム構成:
...
export const calculateC12Metadata: CalculateMetadataFunction<
  C12Props
> = async ({ props }) => {
  try {
    const res = await fetch(staticFile("c12/meta.json"));
...
export const C12ConfidenceInterval: React.FC<C12Props> = ({
  lang = "ja",
  _meta,
}) => {
  const t = I18N[lang].C12;
  ...
  if (!_meta) {
    return (
      ...
      >
        ...
        >
          {"pnpm capture:c12 を実行してください"}
          <br />
          {"(cd video/playwright && pnpm capture:c12)"}
        </p>
      </AbsoluteFill>
    );
  }
  ...
  return (
    <AbsoluteFill>
      ...
      <Sequence from={recordingFrom} durationInFrames={recordingFrames}>
        <FrameSequence sceneId="c12" meta={_meta} />
      </Sequence>
